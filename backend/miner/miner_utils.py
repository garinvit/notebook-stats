...
while True:
    ...
    device = response.json().get("devices")[0]
    ...
    sleep(1)
    response = requests.request("POST", server, headers=headers, data=json.dumps(response))
    if ECHO:
        data = response.json()
        # pprint(data)
        ...
        print(f"OK:{time}: Speed: {speed}, Power: {power}")
    sleep(300)
